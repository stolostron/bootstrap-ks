...
if [ -z "${KUBERNETES_VERSION_OVERRIDE}" ]; then
    ...
    printf "${BLUE}Using KUBERNETES_VERSION_OVERRIDE as the iks version.${CLEAR}\n"
...
printf "${BLUE}Using version ${KUBERNETES_VERSION} for IKS/ROKS cluster.  Nodes will be flavor ${FLAVOR}.${CLEAR}\n"


#----CREATE IKS CLUSTER----#
IKS_CLUSTER_NAME="${RESOURCE_NAME}-${NAME_SUFFIX}"
printf "${BLUE}Creating an IKS cluster named ${IKS_CLUSTER_NAME}.${CLEAR}\n"
...
if [ "$?" -ne 0 ]; then
    printf "${RED}Failed to provision IKS cluster. See error above. Exiting${CLEAR}\n"
    exit 1
fi

TIMEOUT=${TIMEOUT:-"2700"}
printf "${BLUE}Polling for $TIMEOUT seconds for the IKS cluster to be ready.${CLEAR}\n"
acc=0
...
while [[ "${cluster_status}" != "normal" ]]; do
    ...
    if [ "$acc" -ge "$TIMEOUT" ]; then
        printf "${RED}Timed out waiting for IKS clsuter to report 'normal' status.  Final status was $cluster_status.\n${CLEAR}";
...
printf "${GREEN}Cluster reached 'normal' status. Successfully provisioned IKS cluster ${IKS_CLUSTER_NAME}.${CLEAR}\n"
...
if [ "$?" -ne 0 ]; then
    printf "${RED}Failed to get credentials for IKS cluster ${IKS_CLUSTER_NAME}, complaining and continuing${CLEAR}\n"
    exit 1
fi
...
EOF
printf "${GREEN}IKS cluster provision successful.  Cluster named ${IKS_CLUSTER_NAME} created. \n"
